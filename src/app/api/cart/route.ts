...
export async function getCartItemList() {
	const res = await fetch(`${process.env.NEXT_APP_API_PATH}/cart`);
	const data = await res.json();
	return data;
}
export async function getCartItemCount() {
	const res = await fetch(`${process.env.NEXT_APP_API_PATH}/cart?type=count`);
	const data = await res.json();
	return data;
}
// export async function getCartItemAddable(productNo: number) {
//   const res = await fetch(`${process.env.NEXT_APP_API_PATH}/cart?type=addable&productNo=${productNo}`);
//   const data = await res.json();
//   return data;
// }
...
export async function deleteCartItems(productNoList: number[]) {
	const URL = `${process.env.NEXT_APP_API_PATH}/cart`;
